    ...
    return (
        <>
            ...
            <div className="fixed inset-0 flex items-center justify-center z-50">
                <div className="bg-green-300 p-6 rounded-md shadow-lg w-[600px] h-[400px] border border-gray-300">
                    <h2 className="text-3xl font-bold mb-4 text-gray-800 text-center">Contact Info</h2>
                    <div className="space-y-6"> {/* Increased margin between rows */}
                        {Object.entries(contactInfo).map(([label, value], index) => (
                            ...
                            >
                                <strong className="ml-20 text-gray-800 text-lg">
                                    {label.charAt(0).toUpperCase() + label.slice(1)}:
                                </strong>

                                {label === "password" ? (
                                    <span className="flex items-center text-gray-700 mr-8 text-lg">
                                        {value}
                                        <FaEdit
                                    ...
                                    <span className="text-gray-700 mr-8 text-lg">{value}</span>
                                )}
                            </div>
                        ))}
                    </div>
